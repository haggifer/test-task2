...
export default function Header(): ReactElement {
  ...
  const [currentOpenedSection, setCurrentOpenedSection] = useState<string | null>(null)
  ...
  return (
    <CenteredContent
      ...
      wrapperStyles={{
        ...
        boxShadow: breakpoints.down('md') && !showMobileNav ?
          '0px 0px 2px #00000012' : undefined,
      }}
          ...
          <ul className={classes.nav_desktop_list}>
            {
              navLinks.map(link => (
                <li key={link.name} className={classNames(
                  ...
                  <div>
                    <NavLink to={link.href} className={({ isActive }) => classNames(
                    ...
                    {
                      !!link.children &&
                      <i className="icon-arrow" onClick={() => {
                    ...
                    <ul className={classes.nav_desktop_link_children}>
                      {
                        link.children.map(child => (
                          <li key={child.name}>
                            <NavLink to={child.href} className={({ isActive }) => classNames(
                              classes.nav_desktop_link,
                              isActive && classes.active,
                            )}>
                              {child.title}
